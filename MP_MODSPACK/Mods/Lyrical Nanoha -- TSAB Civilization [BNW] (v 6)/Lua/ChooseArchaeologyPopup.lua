...
--Nanoha Mod by Vicevirtuoso (start)
include( "Nanoha_ArchHelper.lua");
--Nanoha Mod by Vicevirtuoso (end)
...
--Nanoha mod by Vicevirtuoso (start)
--Keeps track of whether the artifact is a Logia
local bIsLostLogia = false;
--Nanoha mod by Vicevirtuoso (end)
...
function RefreshList()
	...
	local archeologyUnit;

	--Nanoha Mod by Vicevirtuoso (start)
	if pPlayer:GetCivilizationType() == GameInfoTypes.CIVILIZATION_TSAB then
		archeologyUnit = GameInfo.Units["UNIT_LOST_LOGIA_TEAM"]
	else
		archeologyUnit = GameInfo.Units["UNIT_ARCHAEOLOGIST"];
	end
	--Nanoha Mod by Vicevirtuoso (end)
	...
	if MapModData.Nanoha.LostLogiaPlots[iKey] then
		for i = 0, pPlot:GetNumUnits() do
			local pTestUnit = pPlot:GetUnit(i)
			if pTestUnit:IsHasPromotion(GameInfoTypes.PROMOTION_FIND_LOST_LOGIA) then
				dprint("Lost Logia finder dug up a site!")
					bIsLostLogia = true
					bWrittenArtifact = false
					bShow2ndPlayer = false
					pLostLogiaUnitToDelete = pTestUnit
					break
			else
				ClearLLToDelete()
			end
			if pPlot:GetNumUnits() == i + 1 then
				break
			end
		end
	else
		dprint("Site is not a Lost Logia site")
	end
	
	if (bIsLostLogia) then
		szTypeString = Locale.ConvertTextKey("TXT_KEY_CHOOSE_LOST_LOGIA");
		szDetails1String = Locale.ConvertTextKey("TXT_KEY_CHOOSE_LOST_LOGIA_LINE2")
		szDetails2String = ""
		szDetails3String = Locale.ConvertTextKey("TXT_KEY_CHOOSE_LOST_LOGIA_LINE3")
		szDetails4String = ""
	elseif (iType == GameInfo.GreatWorkArtifactClasses.ARTIFACT_BATTLE_RANGED.ID) then
	--Nanoha Mod by Vicevirtuoso (end)
	...
	--Nanoha Mod by Vicevirtuoso (start)
	-- First choice: for Lost Logia
	if (bIsLostLogia and bArtSlotOpen) then
		local itemInstance = g_ItemManager:GetInstance();
		itemInstance.Name:LocalizeAndSetText("TXT_KEY_CHOOSE_ARCH_LOST_LOGIA_HEADER");
		itemInstance.Description:LocalizeAndSetText("TXT_KEY_CHOOSE_ARCH_LOST_LOGIA_RESULT");	
	...
	if (not bWrittenArtifact and bArtSlotOpen and not bIsLostLogia) then
	--Nanoha Mod by Vicevirtuoso (end)
		local itemInstance = g_ItemManager:GetInstance();
	...
	--Nanoha Mod by Vicevirtuoso (start)
	if (bWrittenArtifact and bWritingSlotOpen and not bIsLostLogia) then
	--Nanoha Mod by Vicevirtuoso (end)
		local itemInstance = g_ItemManager:GetInstance();
	...
	--Nanoha Mod by Vicevirtuoso (start)
	if (bIsLostLogia and not bArtSlotOpen) then
		local itemInstance = g_ItemManager:GetInstance();
		itemInstance.Name:LocalizeAndSetText("TXT_KEY_CHOOSE_ARCH_RENAISSANCE_HEADER");
		local strCultureText = Locale.ConvertTextKey("TXT_KEY_CHOOSE_LOST_LOGIA_RENAISSANCE_RESULT");
	...
	elseif (not bWrittenArtifact and not bIsLostLogia) then
	--Nanoha Mod by Vicevirtuoso (end)
		local itemInstance = g_ItemManager:GetInstance();
...
function OnConfirmYes( )
	...
	--Nanoha Mod by Vicevirtuoso (start)
	if (g_iChoice == 1 and bIsLostLogia == true) then
		dprint("Calling InitLostLogia from ChooseArchaeologyPopup")
		InitLostLogia(Players[Game:GetActivePlayer()])
	end
	--Nanoha Mod by Vicevirtuoso (end)
