...
SELECT     	('LEADER_VV_NOIRE'),			('POLICY_INDUSTRIOUS_X'),	('POLICY_INVENTIVE_X')
WHERE EXISTS (SELECT * FROM Policies WHERE Type = 'POLICY_INDUSTRIOUS_X');

INSERT OR REPLACE INTO Leader_SharedTraits
            (LeaderType,					TraitOne,					TraitTwo)
SELECT     	('LEADER_VV_BLACK_HEART'),		('POLICY_INDUSTRIOUS_X'),	('POLICY_INVENTIVE_X')
WHERE EXISTS (SELECT * FROM Policies WHERE Type = 'POLICY_INDUSTRIOUS_X');

INSERT OR REPLACE INTO Leader_SharedTraits
            (LeaderType,					TraitOne,					TraitTwo)
SELECT     	('LEADER_VV_NOIRE_ULTRA'),		('POLICY_INDUSTRIOUS_X'),	('POLICY_INVENTIVE_X')
WHERE EXISTS (SELECT * FROM Policies WHERE Type = 'POLICY_INDUSTRIOUS_X');

INSERT OR REPLACE INTO Leader_SharedTraits
            (LeaderType,					TraitOne,					TraitTwo)
SELECT     	('LEADER_VV_BLACK_HEART_ULTRA'),('POLICY_INDUSTRIOUS_X'),	('POLICY_INVENTIVE_X')
WHERE EXISTS (SELECT * FROM Policies WHERE Type = 'POLICY_INDUSTRIOUS_X');
...
SET Text = (CASE WHEN EXISTS(SELECT Type FROM Policies WHERE Type = 'POLICY_INDUSTRIOUS_X' )
	THEN 'Noire [ICON_PRODUCTION][ICON_RESEARCH]'
	ELSE 'Noire' END) 
WHERE Tag IN ('TXT_KEY_LEADER_VV_NOIRE', 'TXT_KEY_LEADER_VV_NOIRE_ULTRA');

UPDATE Language_en_US 
SET Text = (CASE WHEN EXISTS(SELECT Type FROM Policies WHERE Type = 'POLICY_INDUSTRIOUS_X' )
	THEN 'Black Heart [ICON_PRODUCTION][ICON_RESEARCH]'
	ELSE 'Black Heart' END) 
WHERE Tag IN ('TXT_KEY_LEADER_VV_BLACK_HEART', 'TXT_KEY_LEADER_VV_BLACK_HEART_ULTRA');
...
SET ArtStyleSuffix = (CASE WHEN EXISTS(SELECT ArtStyleSuffix FROM Civilizations WHERE ArtStyleSuffix = '_JAPAN' )
	THEN '_JAPAN'
	ELSE '_ASIA' END) 
WHERE Type LIKE ('CIVILIZATION_VV_LASTATION%');
...
VALUES		('CIVILIZATION_VV_LASTATION',			'JFD_Gamindustri'),
			('CIVILIZATION_VV_LASTATION_BH',		'JFD_Gamindustri'),
			('CIVILIZATION_VV_LASTATION_ULTRA',		'JFD_Gamindustri'),
			('CIVILIZATION_VV_LASTATION_BH_ULTRA',	'JFD_Gamindustri');
...
-- VALUES		('CIVILIZATION_JAPAN',		'CIVILIZATION_VV_LASTATION'),
			-- ('CIVILIZATION_VV_LASTATION',		'CIVILIZATION_JAPAN'),
			-- ('CIVILIZATION_AMERICA',		'CIVILIZATION_VV_LASTATION'),
			-- ('CIVILIZATION_VV_LASTATION',		'CIVILIZATION_AMERICA');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,				RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_VV_LASTATION'),	('CIVILIZATION_PLANEPTUNE')
-- WHERE EXISTS (SELECT Type FROM Civilizations WHERE Type = 'CIVILIZATION_PLANEPTUNE');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,					RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_PLANEPTUNE'),		('CIVILIZATION_VV_LASTATION')
-- WHERE EXISTS (SELECT Type FROM Civilizations WHERE Type = 'CIVILIZATION_JFD_GREAT_BRITAIN');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,				RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_LASTATION'),		('CIVILIZATION_VV_LASTATION')
-- WHERE EXISTS (SELECT Type FROM Civilizations WHERE Type = 'CIVILIZATION_JFD_AMERICA_HENRY');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,					RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_VV_LASTATION'),		('CIVILIZATION_LASTATION')
...
VALUES		('LEADER_VV_NOIRE',				'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',	9),
			('LEADER_VV_BLACK_HEART',		'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',	9),
			('LEADER_VV_NOIRE_ULTRA',		'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',	9),
			('LEADER_VV_BLACK_HEART_ULTRA',	'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',	9);
...
INSERT INTO Leader_Flavors
			(LeaderType,					FlavorType,				Flavor)
VALUES		('LEADER_VV_NOIRE',				'FLAVOR_JFD_MERCENARY',	1),
			('LEADER_VV_BLACK_HEART',		'FLAVOR_JFD_MERCENARY',	1),
			('LEADER_VV_NOIRE_ULTRA',		'FLAVOR_JFD_MERCENARY',	2),
			('LEADER_VV_BLACK_HEART_ULTRA',	'FLAVOR_JFD_MERCENARY',	2);