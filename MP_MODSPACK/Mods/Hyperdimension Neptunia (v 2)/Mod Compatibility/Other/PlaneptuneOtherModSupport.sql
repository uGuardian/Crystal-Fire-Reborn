...
SELECT     	('LEADER_VV_NEPTUNE'),			('POLICY_CHARISMATIC_X'),	('POLICY_DIPLOMATIC_X')
WHERE EXISTS (SELECT * FROM Policies WHERE Type = 'POLICY_CHARISMATIC_X');

INSERT OR REPLACE INTO Leader_SharedTraits
            (LeaderType,					TraitOne,					TraitTwo)
SELECT     	('LEADER_VV_PURPLE_HEART'),		('POLICY_CHARISMATIC_X'),	('POLICY_DIPLOMATIC_X')
WHERE EXISTS (SELECT * FROM Policies WHERE Type = 'POLICY_CHARISMATIC_X');
...
SET Text = (CASE WHEN EXISTS(SELECT Type FROM Policies WHERE Type = 'POLICY_CREATIVE_X' )
	THEN 'Neptune [ICON_HAPPINESS_1][ICON_INFLUENCE]'
	ELSE 'Neptune' END) 
WHERE Tag = 'TXT_KEY_LEADER_VV_NEPTUNE';

UPDATE Language_en_US 
SET Text = (CASE WHEN EXISTS(SELECT Type FROM Policies WHERE Type = 'POLICY_CREATIVE_X' )
	THEN 'Purple Heart [ICON_HAPPINESS_1][ICON_INFLUENCE]'
	ELSE 'Purple Heart' END) 
WHERE Tag = 'TXT_KEY_LEADER_VV_PURPLE_HEART';
...
SET ArtStyleSuffix = (CASE WHEN EXISTS(SELECT ArtStyleSuffix FROM Civilizations WHERE ArtStyleSuffix = '_JAPAN' )
	THEN '_JAPAN'
	ELSE '_ASIA' END) 
WHERE Type LIKE ('CIVILIZATION_VV_PLANEPTUNE%');
...
VALUES		('CIVILIZATION_VV_PLANEPTUNE',			'JFD_Gamindustri'),
			('CIVILIZATION_VV_PLANEPTUNE_PH',		'JFD_Gamindustri');
...
-- VALUES		('CIVILIZATION_JAPAN',		'CIVILIZATION_VV_PLANEPTUNE'),
			-- ('CIVILIZATION_VV_PLANEPTUNE',		'CIVILIZATION_JAPAN'),
			-- ('CIVILIZATION_AMERICA',		'CIVILIZATION_VV_PLANEPTUNE'),
			-- ('CIVILIZATION_VV_PLANEPTUNE',		'CIVILIZATION_AMERICA');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,				RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_VV_PLANEPTUNE'),	('CIVILIZATION_PLANEPTUNE')
-- WHERE EXISTS (SELECT Type FROM Civilizations WHERE Type = 'CIVILIZATION_PLANEPTUNE');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,					RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_PLANEPTUNE'),		('CIVILIZATION_VV_PLANEPTUNE')
-- WHERE EXISTS (SELECT Type FROM Civilizations WHERE Type = 'CIVILIZATION_JFD_GREAT_BRITAIN');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,				RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_LASTATION'),		('CIVILIZATION_VV_PLANEPTUNE')
-- WHERE EXISTS (SELECT Type FROM Civilizations WHERE Type = 'CIVILIZATION_JFD_AMERICA_HENRY');

-- INSERT INTO Civilization_JFD_RevolutionaryCivilizationsToSpawn
			-- (CivilizationType,					RevolutionaryCivilizationType)
-- SELECT		('CIVILIZATION_VV_PLANEPTUNE'),		('CIVILIZATION_LASTATION')
...
VALUES		('LEADER_VV_NEPTUNE',				'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',	9),
			('LEADER_VV_PURPLE_HEART',			'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',	9);
...
INSERT INTO Leader_Flavors
			(LeaderType,						FlavorType,				Flavor)
VALUES		('LEADER_VV_NEPTUNE',				'FLAVOR_JFD_MERCENARY',	3),
			('LEADER_VV_PURPLE_HEART',			'FLAVOR_JFD_MERCENARY',	0);