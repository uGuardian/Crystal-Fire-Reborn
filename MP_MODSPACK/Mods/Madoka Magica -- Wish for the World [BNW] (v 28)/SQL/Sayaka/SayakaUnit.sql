...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'),	('TXT_KEY_UNIT_PMMM_CUTLASS_CONDUCTOR'),	('TXT_KEY_UNIT_PMMM_CUTLASS_CONDUCTOR_TEXT'),	('TXT_KEY_UNIT_PMMM_CUTLASS_CONDUCTOR_STRATEGY'),	('TXT_KEY_UNIT_PMMM_CUTLASS_CONDUCTOR_HELP'),
			Requirements, Combat, RangedCombat, Cost, FaithCost, RequiresFaithPurchaseEnabled, PurchaseOnly, MoveAfterPurchase, Moves, Immobile, Range, BaseSightRange, Class, Special, Capture, CombatClass, Domain,
			CivilianAttackPriority, DefaultUnitAI, Food, NoBadGoodies, RivalTerritory, MilitarySupport, MilitaryProduction, Pillage, PillagePrereqTech, Found, FoundAbroad, CultureBombRadius, GoldenAgeTurns, FreePolicies,
			OneShotTourism, OneShotTourismPercentOthers, IgnoreBuildingDefense, PrereqResources, Mechanized, Suicide, CaptureWhileEmbarked, PrereqTech, ObsoleteTech, GoodyHutUpgradeUnitClass, HurryCostModifier, AdvancedStartCost,
			MinAreaSize, AirInterceptRange, AirUnitCap, NukeDamageLevel, WorkRate, NumFreeTechs, BaseBeakersTurnsToCount, BaseCultureTurnsToCount, RushBuilding, BaseHurry, HurryMultiplier, BaseGold, NumGoldPerEra,
			...
			ProjectPrereq,SpaceshipProject,LeaderPromotion,LeaderExperience,DontShowYields,ShowInPedia,MoveRate,0,PortraitIndex,IconAtlas,('CIV_ALPHA_ATLAS_SAYAKA')
FROM Units WHERE (Type = 'UNIT_LONGSWORDSMAN');	

--Update Obsolete Tech from Musketman

UPDATE Units
SET ObsoleteTech = (SELECT ObsoleteTech FROM Units WHERE Type = 'UNIT_MUSKETMAN')
WHERE Type = 'UNIT_PMMM_CUTLASS_CONDUCTOR';
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	SelectionSound, FirstSelectionSound
...
INSERT INTO Unit_AITypes 	
			(UnitType, 				UnitAIType)
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	UnitAIType
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	FlavorType, Flavor
...
INSERT INTO Unit_Builds 	
			(UnitType, 				BuildType)
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), BuildType
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	PromotionType
FROM Unit_FreePromotions WHERE (UnitType = 'UNIT_LONGSWORDSMAN');

INSERT INTO Unit_FreePromotions 	
			(UnitType, 							PromotionType)
VALUES		('UNIT_PMMM_CUTLASS_CONDUCTOR', 	'PROMOTION_PMMM_CUTLASS_CONDUCTOR');
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	BuildingType
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	BuildingClassType
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	BuildingType,	ProductionModifier
...
-- Unit_ClassUpgrades
--==========================================================================================================================
--Upgrade class from Musketmen!
INSERT INTO Unit_ClassUpgrades	
			(UnitType, 				UnitClassType)
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	UnitClassType
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	GreatPersonType
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	ResourceType,	Cost
...
WHERE UnitType = 'UNIT_PMMM_CUTLASS_CONDUCTOR' AND ResourceType = 'RESOURCE_IRON';
...
SELECT		('UNIT_PMMM_CUTLASS_CONDUCTOR'), 	YieldType,	Yield
