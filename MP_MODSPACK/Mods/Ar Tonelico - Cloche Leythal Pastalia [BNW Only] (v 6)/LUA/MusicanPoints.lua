...
print ("loaded")
include( "SaveUtils.lua" );MY_MOD_NAME = "MusicanPoints";

local iPromotion = GameInfoTypes.PROMOTION_PASTALIA_MUSIC
local iMusician = GameInfoTypes.SPECIALIST_MUSICIAN

function MusicanPoints(iPlayer)
	...
	if pTargetCity ~= nil then
		iMod = iMod + (GetGreatPersonModifier(pPlayer, pTargetCity, iMusician))
	end
		...
		if pUnit:IsHasPromotion(iPromotion) then
			...
			if pTargetCity ~= nil then
				...
				if iOldXP ~= nil then
					...
					if iDelta > 0 then
						pTargetCity:ChangeSpecialistGreatPersonProgressTimes100(iMusician, iDelta)
					end
				end
			end
		end
...
GameEvents.PlayerDoTurn.Add(MusicanPoints)
...
function GetTargetCity(pPlayer)
	...
	local iTargetProgress = pTargetCity:GetSpecialistGreatPersonProgress(iMusician)
	for pCity in pPlayer:Cities() do
		local iProgress = pCity:GetSpecialistGreatPersonProgress(iMusician)
