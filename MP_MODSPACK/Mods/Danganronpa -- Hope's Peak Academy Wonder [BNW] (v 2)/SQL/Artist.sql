...
VALUES		('UNITCLASS_VV_SHSL_ARTIST',	'TXT_KEY_UNIT_VV_SHSL_ARTIST',	'UNIT_VV_SHSL_ARTIST');
...
SELECT		('UNIT_VV_SHSL_ARTIST'),	('TXT_KEY_UNIT_VV_SHSL_ARTIST'),	Civilopedia,	Strategy,	('TXT_KEY_UNIT_VV_SHSL_ARTIST_HELP'),
			Requirements, Combat, RangedCombat, Cost, FaithCost, RequiresFaithPurchaseEnabled, PurchaseOnly, MoveAfterPurchase, Moves, Immobile, Range, BaseSightRange, ('UNITCLASS_VV_SHSL_ARTIST'), Special, Capture, CombatClass, Domain,
			CivilianAttackPriority, DefaultUnitAI, Food, NoBadGoodies, RivalTerritory, MilitarySupport, MilitaryProduction, Pillage, PillagePrereqTech, Found, FoundAbroad, CultureBombRadius, GoldenAgeTurns, FreePolicies,
			OneShotTourism, OneShotTourismPercentOthers, IgnoreBuildingDefense, PrereqResources, Mechanized, Suicide, CaptureWhileEmbarked, PrereqTech, ObsoleteTech, GoodyHutUpgradeUnitClass, HurryCostModifier, AdvancedStartCost,
			MinAreaSize, AirInterceptRange, AirUnitCap, NukeDamageLevel, WorkRate, NumFreeTechs, BaseBeakersTurnsToCount, BaseCultureTurnsToCount, RushBuilding, BaseHurry, HurryMultiplier, BaseGold, NumGoldPerEra,
			SpreadReligion, RemoveHeresy,ReligionSpreads,ReligiousStrength,FoundReligion,RequiresEnhancedReligion,ProhibitsSpread,CanBuyCityState,CombatLimit,RangeAttackOnlyInDomain,RangeAttackIgnoreLOS,Trade,NumExoticGoods,
...
UPDATE Units
SET GoldenAgeTurns	= CAST((GoldenAgeTurns*0.5) AS INT)
WHERE Type = 'UNIT_VV_SHSL_ARTIST';
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	SelectionSound, FirstSelectionSound
...
INSERT INTO Unit_AITypes 	
			(UnitType, 				UnitAIType)
SELECT		('UNIT_VV_SHSL_ARTIST'), 	UnitAIType
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	FlavorType, Flavor * 20
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	PromotionType
FROM Unit_FreePromotions WHERE (UnitType = 'UNIT_ARTIST');
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	BuildingType
FROM Unit_Buildings WHERE (UnitType = 'UNIT_ARTIST');
...
VALUES		('UNIT_VV_SHSL_ARTIST','BUILDINGCLASS_VV_KIBOUGAMINE');
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	BuildingType,	ProductionModifier
FROM Unit_ProductionModifierBuildings WHERE (UnitType = 'UNIT_ARTIST');
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	UnitClassType
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	GreatPersonType
FROM Unit_GreatPersons WHERE (UnitType = 'UNIT_ARTIST');
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	ResourceType,	Cost
FROM Unit_ResourceQuantityRequirements WHERE (UnitType = 'UNIT_ARTIST');
...
SELECT		('UNIT_VV_SHSL_ARTIST'), 	YieldType,	Yield
FROM Unit_YieldFromKills WHERE (UnitType = 'UNIT_ARTIST');
...
INSERT INTO Unit_UniqueNames
			(UnitType,					UniqueName)
SELECT		('UNIT_VV_SHSL_ARTIST'),	Tag
