...
local g_ListMode_Flight_Plan = 10;
...
g_tListModeText[g_ListMode_Flight_Plan] = "TXT_KEY_PROGRESS_SCREEN_FLIGHT_PLAN";
...
local g_tListOfDudes = {
									...
									"TXT_KEY_DUDE_6",
									"TXT_KEY_DUDE_7",
									"TXT_KEY_BIG_DUDE_1" -- Vicevirtuoso
								};
...
function OnPopup( popupInfo )
	...
	--Vicevirtuoso: CIA always does the flight plan
	if strDudesName == "TXT_KEY_BIG_DUDE_1" then
		g_iListMode = g_ListMode_Flight_Plan
	else
	-- Keep rolling until we get a new mode
		while iRand == g_iLastListMode or bInvalid do
			...
			if (not IsListModeAllowed(iRand)) or iRand == g_ListMode_Flight_Plan then
				bInvalid = true;
			end
	...
	--Vicevirtuoso: Flight Plan
	elseif (g_iListMode == g_ListMode_Flight_Plan) then
		for i = 0, 7, 1 do
			local controlTable = g_PlayerListInstanceManager:GetInstance();   
			local strName = Locale.ConvertTextKey("TXT_KEY_PROGRESS_SCREEN_NUMBERING", i).." "..Locale.ConvertTextKey("TXT_KEY_SPY_NAME_VV_CIA_"..i);
			IconHookup( leader.PortraitIndex, 64, leader.IconAtlas, controlTable.Portrait );
			controlTable.PlayerNameText:SetText(strName);
			controlTable.ScoreText:SetText("");
		end
