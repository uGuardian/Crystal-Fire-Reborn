...
function UpdateUnitPortrait( pUnit )
	...
	--PMMM Mod
	local name;
	if pUnit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_MAGICAL_GIRL or (pUnit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_WITCH and pUnit:HasName()) then
	...
	if pUnit:GetUnitClassType() ~= GameInfoTypes.UNITCLASS_PMMM_MAGICAL_GIRL then
		...
		Controls.PMMMBodyPortrait:SetHide(true);
		Controls.PMMMOutfitPortrait:SetHide(true);
		Controls.PMMMFacePortrait:SetHide(true);
		Controls.PMMMHairPortrait:SetHide(true);
		...
		for k, v in ipairs(MapModData.PMMM.MagicalGirls) do
			if v.UnitID == pUnit:GetID() and v.Owner == pUnit:GetOwner() then
				iMGKey = k
				break
			end
		end
		if MapModData.PMMM.MagicalGirls[iMGKey].OverrideIcon then
			textureOffset, textureAtlas = IconLookup( MapModData.PMMM.MagicalGirls[iMGKey].OverrideIcon[2], g_iPortraitSize, MapModData.PMMM.MagicalGirls[iMGKey].OverrideIcon[1]);
			if textureOffset == nil then
				textureOffset = nullOffset;
			...
			if g_AppearanceChanges[1] then
				textureOffset, textureAtlas = IconLookup( g_AppearanceChanges[1][2], g_iPortraitSize, g_AppearanceChanges[1][1]);
				if textureOffset ~= nil then
					Controls.PMMMBodyPortrait:SetTexture(textureAtlas);
					Controls.PMMMBodyPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMBodyPortrait:SetHide(false);
				else
					Controls.PMMMBodyPortrait:SetHide(true);
				end
			elseif MapModData.PMMM.MagicalGirls[iMGKey].BodyIcon then
				textureOffset, textureAtlas = IconLookup( MapModData.PMMM.MagicalGirls[iMGKey].BodyIcon[2], g_iPortraitSize, MapModData.PMMM.MagicalGirls[iMGKey].BodyIcon[1]);
				if textureOffset ~= nil then
					Controls.PMMMBodyPortrait:SetTexture(textureAtlas);
					Controls.PMMMBodyPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMBodyPortrait:SetHide(false);
			...
			if g_AppearanceChanges[2] then
				textureOffset, textureAtlas = IconLookup( g_AppearanceChanges[2][2], g_iPortraitSize, g_AppearanceChanges[2][1]);
				if textureOffset ~= nil then
					Controls.PMMMBodyPortrait:SetTexture(textureAtlas);
					Controls.PMMMBodyPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMBodyPortrait:SetHide(false);
			...
			elseif MapModData.PMMM.MagicalGirls[iMGKey].OutfitIcon then
				textureOffset, textureAtlas = IconLookup( MapModData.PMMM.MagicalGirls[iMGKey].OutfitIcon[2], g_iPortraitSize, MapModData.PMMM.MagicalGirls[iMGKey].OutfitIcon[1]);
				if textureOffset ~= nil then
					Controls.PMMMOutfitPortrait:SetTexture(textureAtlas);
					Controls.PMMMOutfitPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMOutfitPortrait:SetHide(false);
				else
					Controls.PMMMOutfitPortrait:SetHide(true);
				end
			end
			if g_AppearanceChanges[3] then
				textureOffset, textureAtlas = IconLookup( g_AppearanceChanges[3][2], g_iPortraitSize, g_AppearanceChanges[3][1]);
				if textureOffset ~= nil then
					Controls.PMMMBodyPortrait:SetTexture(textureAtlas);
					Controls.PMMMBodyPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMBodyPortrait:SetHide(false);
			...
			elseif MapModData.PMMM.MagicalGirls[iMGKey].FaceIcon then
				textureOffset, textureAtlas = IconLookup( MapModData.PMMM.MagicalGirls[iMGKey].FaceIcon[2], g_iPortraitSize, MapModData.PMMM.MagicalGirls[iMGKey].FaceIcon[1]);
				if textureOffset ~= nil then
					Controls.PMMMFacePortrait:SetTexture(textureAtlas);
					Controls.PMMMFacePortrait:SetTextureOffset(textureOffset);
					Controls.PMMMFacePortrait:SetHide(false);
				else
					Controls.PMMMFacePortrait:SetHide(true);
				end
			end
			if g_AppearanceChanges[4] then
				textureOffset, textureAtlas = IconLookup( g_AppearanceChanges[4][2], g_iPortraitSize, g_AppearanceChanges[4][1]);
				if textureOffset ~= nil then
					Controls.PMMMBodyPortrait:SetTexture(textureAtlas);
					Controls.PMMMBodyPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMBodyPortrait:SetHide(false);
			...
			elseif MapModData.PMMM.MagicalGirls[iMGKey].HairIcon then
				textureOffset, textureAtlas = IconLookup( MapModData.PMMM.MagicalGirls[iMGKey].HairIcon[2], g_iPortraitSize, MapModData.PMMM.MagicalGirls[iMGKey].HairIcon[1]);
				if textureOffset ~= nil then
					Controls.PMMMHairPortrait:SetTexture(textureAtlas);
					Controls.PMMMHairPortrait:SetTextureOffset(textureOffset);
					Controls.PMMMHairPortrait:SetHide(false);
				else
					Controls.PMMMHairPortrait:SetHide(true);
				end
			end
			textureOffset, textureAtlas = IconLookup( MapModData.gPMMMIconBuilderBackground[2], g_iPortraitSize, MapModData.gPMMMIconBuilderBackground[1]);
			Controls.UnitPortrait:SetTexture(textureAtlas);
			Controls.UnitPortrait:SetTextureOffset(textureOffset);
		end
	end
...
function UpdateUnitStats(pUnit)
	...
	--PMMM Mod: Soul Gem
	elseif pUnit:GetUnitClassType() == GameInfoTypes.UNITCLASS_PMMM_MAGICAL_GIRL then
		local iMGKey;
		for k, v in pairs(MapModData.PMMM.MagicalGirls) do
			if v.UnitID == iUnit and v.Owner == iPlayer then
				iMGKey = k
			end
		end
		if iMGKey then
			local iCorruption = MapModData.PMMM.MagicalGirls[iMGKey].SoulGem or 0
			local iPolish = MapModData.PMMM.MagicalGirls[iMGKey].Polish or 0
...
--PMMM Time Stop Promos
local tTimeStopPromotions = {}
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			--PMMM Mod: Incubators and Artificial Incubators deal no melee damage
			local sTheirType = GameInfo.Units[pTheirUnit:GetUnitType()].Type
			...
			-- Normal Melee Combat
				...
				--PMMM Mod: Incubators and Artificial Incubators deal no melee damage
				if string.find(sTheirType, "_PMMM_") and string.find(sTheirType, "INCUBATOR") then
			...
			if (eCombatPrediction == CombatPredictionTypes.COMBAT_PREDICTION_RANGED) then
				...
				Controls.RangedAttackButtonLabel:SetText(Locale.ToUpper(Locale.ConvertTextKey("TXT_KEY_INTERFACEMODE_RANGE_ATTACK")));
			--PMMM Mod
			elseif (string.find(sTheirType, "_PMMM_") and string.find(sTheirType, "INCUBATOR")) or HasAnyTimeStopPromo(pTheirUnit) then
				Controls.RangedAttackIndicator:SetHide(false);
				Controls.RangedAttackButtonLabel:SetText(Locale.ToUpper(Locale.ConvertTextKey("TXT_KEY_INTERFACEMODE_PMMM_NO_COUNTERATTACK_DAMAGE")));
			--PMMM Mod
			...
			--PMMM Mod: Remove percentages from Soul Gem states and Moods so they can be added in separate lines
			local iSGMods = 0
			local iMoodMods = 0
			for promotion in GameInfo.UnitPromotions() do
				if pMyUnit:IsHasPromotion(promotion.ID) and promotion.CombatPercent ~= 0 then
					if string.find(promotion.Type, "_PMMM_SGSTATE_") then
						iModifier = iModifier - promotion.CombatPercent
						iSGMods = iSGMods + promotion.CombatPercent
					elseif string.find(promotion.Type, "_PMMM_MGMOOD_") then
						iModifier = iModifier - promotion.CombatPercent
						iMoodMods = iMoodMods + promotion.CombatPercent
					end
				end
			end
			--PMMM Mod
			...
			--PMMM Mod: Soul Gem Status & Mood Status
			if (iSGMods ~= 0) then
				controlTable = g_MyCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_PMMM_EUPANEL_SOUL_GEM_STATE" );
				controlTable.Value:SetText( GetFormattedText(strText, iSGMods, true, true) );
			end
			if (iMoodMods ~= 0) then
				controlTable = g_MyCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_PMMM_EUPANEL_MAGICAL_GIRL_MOOD" );
				controlTable.Value:SetText( GetFormattedText(strText, iMoodMods, true, true) );
			end
			--PMMM Mod
...
if (not isBNW) then
	...
	--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_FORTIFY_MOD", iModifier));
			...
			local iMoodMods = 0
			for promotion in GameInfo.UnitPromotions() do
				if pTheirUnit:IsHasPromotion(promotion.ID) and promotion.CombatPercent ~= 0 then
					if string.find(promotion.Type, "_PMMM_SGSTATE_") then
						iModifier = iModifier - promotion.CombatPercent
						iSGMods = iSGMods + promotion.CombatPercent
					elseif string.find(promotion.Type, "_PMMM_MGMOOD_") then
			...
			if (iSGMods ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_PMMM_EUPANEL_SOUL_GEM_STATE" );
				controlTable.Value:SetText( GetFormattedText(strText, iSGMods, false, true) );
			end
			if (iMoodMods ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_PMMM_EUPANEL_MAGICAL_GIRL_MOOD" );
				controlTable.Value:SetText( GetFormattedText(strText, iMoodMods, false, true) );
			end
			--PMMM Mod
